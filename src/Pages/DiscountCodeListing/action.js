...
export const deleteDiscount = ({ id, callback }) => {
  return fetchAction({
    endpoint: `${APP_BASE_URL_4}/DiscountCode/DeleteDiscountCode?id=${id}`,
    verb: "DELETE",
  }).then((res) => {
    console.log(res)
    callback(res);
...
export const searchDiscountCode = ({searchText, callback}) => {
  ...
  return fetchAction({
   endpoint: `${APP_BASE_URL_4}/DiscountCode/SearchDiscountCode?text=${searchText}&CountryId=${getCookies('country').id}`,
